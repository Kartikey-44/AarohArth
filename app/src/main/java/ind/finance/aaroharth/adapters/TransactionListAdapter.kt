...
import ind.finance.aaroharth.modificationsFragments.TransactionModification
...
class TransactionListAdapter(
    ...
    override fun onBindViewHolder(
        ...
        val type = transaction.transactionType.lowercase().trim()
        if (type == "income") {
            holder.iconholder.setBackgroundResource(R.drawable.transaction_card_background_income)
            holder.card.setStrokeColor(
                ContextCompat.getColor(holder.itemView.context, R.color.recent_transaction_card_stroke_income)
            )
            holder.amount.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.recent_tran_amount_income)
            )
            holder.doot.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.income_dot_color)
            )
            holder.dot2.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.income_dot_color)
            )
            ...
            holder.card.setStrokeColor(
                ContextCompat.getColor(holder.itemView.context, R.color.recent_transaction_card_stroke_expense)
            )
            holder.amount.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.recent_tran_amount_expense)
            )
            holder.doot.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.expense_dot_color)
            )
            holder.dot2.setTextColor(
                ContextCompat.getColor(holder.itemView.context, R.color.expense_dot_color)
            )
        ...
        val medium = transaction.transactionMedium.lowercase().trim()
        if (medium == "upi") {
        ...
        when (transaction.category.lowercase()) {
            ...
            "lpg", "png" -> holder.iconholder.setImageResource(R.drawable.lpgpng)
