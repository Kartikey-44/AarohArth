...
class AccountActions : AppCompatActivity() {
    ...
    private fun catchAssistantIntent() {   // ✅ added
        ...
        val balance =
            intent.getLongExtra("balance", -1L)


        if (!accountType.isNullOrBlank()) {

            binding.typeField.setText(
                accountType,
                false
            )

            updateHint(accountType)
        }
    ...
    private fun isInputValid(): Boolean {
        ...
        if (binding.typeField.text.toString()
                .trim()
                .isEmpty()
        ) {
    ...
    private fun showSuccessDialog() {
        ...
        val dialogBinding =
            DialogScreenBinding.inflate(
                layoutInflater
            )

        successDialog!!.setContentView(
            dialogBinding.root
        )
        ...
        successDialog!!.window
            ?.setBackgroundDrawable(
                getDrawable(
                    ind.finance.aaroharth.R.drawable.dialog_background
                )
            )
        ...
        Handler(Looper.getMainLooper())
            .postDelayed({
                ...
                if ((viewModel.accountCount.value
                        ?: 0) <= 1
                ) {
