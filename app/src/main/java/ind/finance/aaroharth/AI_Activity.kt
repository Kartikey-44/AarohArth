...
class AI_Activity : AppCompatActivity() {
    ...
    private fun startSpeech() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.RECORD_AUDIO), SPEECH_REQ)
            return
        }
    ...
    private fun openTransaction(data: AI_ViewModel.ExtractedData) {
        val intent = Intent(this, Transaction_Action_Page::class.java).apply {
            putExtra("type", data.type)
            putExtra("amount", data.amount ?: -1L)
            putExtra("category", data.category)
            putExtra("accountType", data.accountType)
            putExtra("note", data.originalText)
        }
        startActivity(intent)
    }

    private fun openBudget(data: AI_ViewModel.ExtractedData) {
        val intent = Intent(this, BudgetActions::class.java).apply {
            putExtra("amount", data.amount ?: -1L)
            putExtra("category", data.category)
            putExtra("note", data.originalText)
        }
    ...
    private fun openAccount(data: AI_ViewModel.ExtractedData) {
        val intent = Intent(this, AccountActions::class.java).apply {
            putExtra("accountType", data.accountType)
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == SPEECH_REQ && grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
            startSpeech()
        }
    }
}
